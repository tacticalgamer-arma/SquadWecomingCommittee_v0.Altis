params [["_pole",objNull,[objNull]],["_jump_alt",10000,[0]],["_jump_safety",90,[0]]];

TOC_doHALO = compileFinal "
  _plyr = _this;
  TOC_fnc_orient = {
    _obj = _this select 0;
    _p = _this select 1;
    ...
    ['EHid','onEachFrame', {
      ...
      call{
        if (stance _plyr == 'UNDEFINED') exitWith {
          ...
          [_whs,-180]  call TOC_fnc_orient
        };
        if (stance _plyr != 'UNDEFINED') exitWith {
          _whs  attachTo [_plyr,[-0.1,0.75,-0.05],'leaning_axis'];
          [_whs,-90] call TOC_fnc_orient
        };
      };
...
_pole addAction ["<t color='#00ffff'>HALO jump for group (AI)</t>",{
  ...
  ["Jump","onMapSingleClick", {
    ...
    params ["","_pos","","","_jump_alt","_jump_safety","_TOC_forSquad"];
    _caller = player;
    _TOC_units =  (units _caller) select {local _x && alive _x &&  _x distanceSqr _caller < 100000 && isnull objectParent _x};
    {
